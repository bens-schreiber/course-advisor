...
def test_top_professors_snapshot(snapshot, client):
    ...
    snapshot.assert_match(json.dumps(response.get_json(), indent=2, sort_keys=True), "top_professors_response")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"credits": 4, "class_level": 100, "department_id": 2, "ucore": 3}, "top_classes_with_ucore"),
        ({"credits": 3, "class_level": 100, "department_id": 999}, "top_classes_invalid_dept"),
...
def test_top_class_snapshot(snapshot, client, params, snapshot_name):
    ...
    assert response.status_code == 200
    snapshot.assert_match(json.dumps(response.get_json(), indent=2, sort_keys=True), snapshot_name)
